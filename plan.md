...
- [x] Admin UI can configure S3-compatible storage, create backups, schedule backups, download backup files, delete records, and restore completed records.
- [x] Backup records are stored in application settings under `backup_records`.
- [x] Backup files are `pg_dump` SQL dumps compressed as `.sql.gz` and stored in S3-compatible object storage.
...
- [x] Keep restore password confirmation unchanged.
...
- [x] Keep restore flow unchanged.

### Notes

- [ ] This should not load large files fully into memory.
- [x] Current S3 upload implementation reads the whole object into memory, so streaming or multipart upload should be implemented before enabling large browser uploads.
...
- [ ] Implement S3 object import because it reuses the existing restore path and has the smallest risk.
- [ ] Improve S3 upload internals to support streaming or multipart uploads.
- [ ] Add local file upload after upload memory usage is safe.
- [ ] Add safer restore UX and audit trail.
- [ ] Evaluate Redis and full-instance backup after database import is stable.
